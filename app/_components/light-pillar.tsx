...
function parseColorToVector(hex: string) {
...
export default function LightPillar({
  ...
  useEffect(() => {
    ...
    const isLowEndDevice = isMobile || hardwareThreads <= 4;
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uTopColor: { value: parseColorToVector(topColor) },
        uBottomColor: { value: parseColorToVector(bottomColor) },
  ...
  useEffect(() => {
    ...
    materialRef.current.uniforms.uTopColor.value = parseColorToVector(topColor);
  }, [topColor]);
  ...
  useEffect(() => {
    ...
    materialRef.current.uniforms.uBottomColor.value = parseColorToVector(bottomColor);
  }, [bottomColor]);
  ...
  if (!webGLSupported) {
    return (
      <div className={`${styles.fallback} ${className}`.trim()} style={{ mixBlendMode }}>
        WebGL not supported
      </div>
    );
  }
